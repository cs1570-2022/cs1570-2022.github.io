...
\begin{enumerate}[(a)]
    ...
    \item Sketch a divide and conquer recursive algorithm for square matrix multiplication based on the naïve approach. Provide a bound to its runtime using the Master Theorem. Rather than using asymptotic notation, try to analyze the runtime (expecially the combine step) with an exact constant. 
    \item Sketch a divide and conquer recursive algorithm for square matrix multiplication based on Strassen’s approach. Provide a bound to its runtime using the Master Theorem.
...
\item[(a)] Let $f(i)$ be the index of the column containing the leftmost minimum lement of row $i$. Prove that $f(1)\leq f(2)\leq \ldots, f(m)$ for an $m \times n$ Gnome array.
    \item[(b)]Here is a description of a divide-and-conquer algorithm that computes the leftmost minimum element in each row of a $m\times n$ Gnome array
    \begin{itemize}
    ...
    \item[(c)] Write the recurrence describing the running time of the algorithm, and obtain a explicit closed-form solution for it,
\end{enumerate}
\end{problem}
